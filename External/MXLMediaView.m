...
-(void)showMediaFile:(id)mediaFile ofType:(MXLMediaViewType)mediaType inParentView:(UIView *)parentView completion:(void(^)(void))completion {
    ...
    // Set up self
    [self setFrame:CGRectMake(0.0, 0.0, parentView.frame.size.width, parentView.frame.size.height)];
    ...
    [_backgroundImageView setAlpha:0.0];                                   // Make invisible
    ...
    [UIView animateWithDuration:0.2 animations:^{
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            [_backgroundImageView setCenter:CGPointMake([UIScreen mainScreen].bounds.size.width/2.0, [UIScreen mainScreen].bounds.size.height/2.0)];
...
-(void)showMediaImageView {
    ...
    [_mediaImageView setFrame:CGRectMake(0.0, -_mediaImageView.frame.size.height, self.frame.size.width, self.frame.size.height)]; // Set it to be off frame
    ...
    [self dropDownView:_mediaImageView withGravityVelocity:(IS_DEVICE_IPAD ? 9.0 : 2.5)];
}

// Method to show the video player
-(void)showMoviePlayerView {
    _mediaPlayerController = [[MPMoviePlayerController alloc] initWithContentURL:_videoURL];
    [_mediaPlayerController.view setFrame:CGRectMake(0.0, -180.0, self.frame.size.width, 180.0)];
    [_mediaPlayerController.view setContentMode:UIViewContentModeScaleAspectFit];
    
    [self addSubview:_mediaPlayerController.view];
    
    [self dropDownView:_mediaPlayerController.view withGravityVelocity:(IS_DEVICE_IPAD ? 10.0 : 3.5)];
}
...
-(void)dropDownView:(UIView *)view withGravityVelocity:(float)velocity {
    ...
    [collisionBehaviour addBoundaryWithIdentifier:@"barrier"
                                        fromPoint:CGPointMake(0.0, (self.frame.size.height + view.frame.size.height)/2.0)
                                          toPoint:CGPointMake(self.frame.size.width, (self.frame.size.height + view.frame.size.height)/2.0)];
    ...
    [gravityBehaviour setGravityDirection:CGVectorMake(0.0, velocity)];
...
-(void)hideMediaView {
    ...
    [UIView animateWithDuration:0.2 animations:^{
        ...
        [_mediaImageView setCenter:CGPointMake([UIScreen mainScreen].bounds.size.width/2.0, [UIScreen mainScreen].bounds.size.height/2.0)];
        ...
        [_mediaPlayerController.view setCenter:CGPointMake([UIScreen mainScreen].bounds.size.width/2.0, [UIScreen mainScreen].bounds.size.height/2.0)];
    } completion:^(BOOL finished) {
...
-(UIImage *)blurredImageFromView:(UIView *)backgroundView {
    ...
    backgroundImage = [backgroundImage applyBlurWithRadius:6.0 tintColor:[UIColor colorWithWhite:0.0 alpha:0.6] saturationDeltaFactor:1.0 maskImage:nil];
